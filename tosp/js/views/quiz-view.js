...
function renderResult(container, module, result, quiz) {
  ...
  const reviewHtml = result.selectedAnswers ? questions.map((q, i) => {
    ...
    return `
      <li class="quiz-review-row quiz-review-row--${wasCorrect ? 'correct' : 'incorrect'}">
        <p class="quiz-review-row__prompt">${i + 1}. ${q.prompt}</p>
        <p class="muted">Your answer: ${selectedText} ${wasCorrect ? '✓' : '✗'}</p>
  ...
  container.innerHTML = `
    <section class="panel quiz-result quiz-result--${result.passed ? 'pass' : 'fail'}">
...
export function render(container, params) {
  ...
  // refresh (so the header's overall progress reflects it too), instead of
  ...
  form.addEventListener('submit', (e) => {
    ...
    const confirmed = window.confirm('Submit your Skill Check answers now? You cannot change them after submitting.');
    if (!confirmed) return;
    ...
    try {
      ...
      // Goes through the router so the header (overall progress) refreshes
